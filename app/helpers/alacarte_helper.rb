module AlacarteHelper
  ...
  def navigation_for(menu, current_element)
  ...
  def elements_for(menu, current_element)
    ...
    menu.items.each do |item|
      if item.valid?
        ...
        _item = case item.type
          when :link: link_to(item.label, item.path, _html_options)
    ...
    result.blank? ? '' : content_tag(:ul, result.html_safe, menu.html_options)
  end
